...
public class Request {
	...
	public void createTPG(String tpgName, String tpgTags[], String tpgStatus[],String tpgType[]) throws Exception {
		...
		Common.FindAllElements(RequestRepo.moveIconRight, 0);
		}
...
//		Assert.assertEquals(Common.FindAnElement(RequestRepo.SpanWorkFlowStatus).getText(), "Rejected by Requestor","Workflow status is not Approved");
	...
	public void passByLab(String RequestName,String ContainerName,String BrandLimitResult){
		...
		Common.ClickArrowIcon(RequestRepo.ArrowForOpenForm, 0);
		Common.ClickElement(RequestRepo.OptionToOpenForm, 0);
		...
		NewRequestFrame=Common.FindAllElements(RequestRepo.frmFrameNewRequest, 1010);
		if (NewRequestFrame.size()>0)
		{
			if (NewRequestFrame.size()>1)
			{
	...
	{
		Assert.assertEquals(Common.FindAnElement(RequestRepo.SpanWorkFlowStatus, 1000).getText(), "Approved","Workflow status is not Approved");
		Assert.assertEquals(Common.FindAnElement(RequestRepo.SpanWorkFlowStatus, 1000).getText(), "Rejected by Requestor","Workflow status is not Approved");
		Common.SwitchToDefaultContent(0);
	}
	...
	//=============================== Added By R  27.12.16 ==============================================
	// Send Normal Request
	 public void sendRequest(String RequestName,boolean selectTP,String TradingPartnerName,String formNames[], String ProductName,String DocumentName,String SendRequestComments) 
			   throws Exception 
			 {
			  ...
			  if(Common.getElementText(RequestRepo.noDocFormfound, 60).equalsIgnoreCase("No record")){
			   ...
			   Common.ClickElementByIndex(RequestRepo.chooseDocFormIndex1, 1, 120);
				  /*Common.selectFormChk(formNames);*/
			  }
			  ...
			  if(Common.checkExistenceOfElement(By.xpath("//*[@id='sendRequestTable']/slds-table/table/tbody/tr/td[1]/label"), 20))
			  {
				  Common.ClickElement(By.xpath("//*[@id='sendRequestTable']/slds-table/table/tbody/tr/td[1]/label"), 20);
				  Common.ClickElement(By.xpath("//button[contains(.,'Yes')]"), 20);
			  }
			  else{
				  Common.ClickElement(RequestRepo.btnClose, 5);
			  }
			  if(Common.checkExistenceOfElement(RequestRepo.btnClose, 5));{
				  Common.ClickElement(RequestRepo.btnClose, 5);
			  }
			  Common.RefreshPage(5);
			  Common.ClickElement(RequestRepo.refreshRequestList, 60);
			  Log.info("Refreshing Request List");
			  Common.assertTrue(Common.getElementText(RequestRepo.topRequestName, 120), RequestName);
	...
	//Change workflow status to faild and submit form from Lab
	public void changeWorkFlowStatus(String SendRequestComments, String WorkFlowStatus) throws Exception{
		...
		List<WebElement> NewRequestFrame=Common.FindAllElements(RequestRepo.frmFrameNewRequest, 100);
		System.out.println(NewRequestFrame.size());	
		NewRequestFrame=Common.FindAllElements(RequestRepo.frmFrameNewRequest, 0);
		if (NewRequestFrame.size()>0)
		{
			if (NewRequestFrame.size()>1)
			{
	...
	public void FillAndSubmit3ActorForm(String RequestName,String ContainerName,String AttentionComments,String TestingFacility) throws InterruptedException
	{
		...
		Common.ClickElement(RequestRepo.OptionToOpenForm, 0);
		List<WebElement> NewRequestFrame=Common.FindAllElements(RequestRepo.frmFrameNewRequest, 0);
		System.out.println(NewRequestFrame.size());
	...
	public void Verifyquestionresponder(String RequestName,String ContainerName,String AttentionComments ) throws InterruptedException
	{
		...
		List<WebElement> ArrowToOpenMenu = Common.FindAllElements(RequestRepo.ArrowToOpenMenu, 0);
		...
		if (NewRequestFrame.size()>0)
		{
			...
			WebElement a=Common.FindAnElement(RequestRepo.Question, 0);
	...
	public void submitBackToRequestor(String RequestName,String ContainerName,String AttentionComments,String TestingFacility){
		//Common.GlobalSearch("Requests", RequestName);
		Common.GlobalSearch("Workflows", RequestName);
			
 		Common.ClickLink(RequestName, 5000);
		Common.ClickElement(RequestRepo.tabRelated, 2000);
//		Common.ClickElement(RequestRepo.lnkWorkflowName, 2000);
		Common.ClickElement(RequestRepo.btnSubmitWorkflowPage, 2000);
		Common.SwitchToFrame();
		Common.ClickElement(RequestRepo.btnSubmitReqComfirmation, 8000);
