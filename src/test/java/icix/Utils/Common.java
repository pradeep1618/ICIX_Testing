...
//	}
	...
	public static void GlobalSearch(String SearchTab, String KeywordToSearch)
	 {
	  try
	  {
	   closeAppLauncher();
	   ...
	   ConsoleLog.info("Keyword "+ KeywordToSearch + " entered for searching.");
	   Thread.sleep(500);
	   Common.ClickElement(GlobalRepo.optionSearchInSalesforce, 10);
	   Log.info("Clicked on " + KeywordToSearch + " in salesforec.");
	   Thread.sleep(500);
	   Common.ClickElement(GlobalRepo.lnkShowMore, 3000);
	   Log.info("Clicked on show more link.");
	   System.out.println("we got the request type as "+SearchTab);
	   System.out.println("we got the keyword to search as "+KeywordToSearch);
	   switch(SearchTab){
	         case "Workflows":  Common.ClickElement(GlobalRepo.globalSearchRequestTab, 120);
	          WaitUntilResultsPresent(GlobalRepo.globalSearchRequestTab);
	          Log.info("Request tab clicked");
	          break;
	          
	         case "Requests":  Common.ClickElement(GlobalRepo.globalSearchRequestTab, 120);
...
//		driver.quit();
	...
	public static void assertText(By Element,String ExpectedText, int timeOutInSeconds) throws Exception
	{		
		try {
			WaitTool.waitForElementToBeClickable(driver, Element, timeOutInSeconds);
	...
	public static void deleteCookies(int timeOutInSeconds){
		driver.manage().deleteAllCookies();
		
		WaitTool.waitImplicit(driver, timeOutInSeconds);
	}
	
	public static void clickAppLauncher(){
		ClickElement(LoginOutRepo.imgWaffle, 5);
	}
	...
	//Added by Vishal Dhiman 15/02/2017
	 public static void selectDate(By element, int interval) throws Exception {
	        ...
	        List<WebElement> tblTds= Common.FindAllElements(GlobalRepo.tblTd,25);
	        Log.info("Clicking on date");
	        WaitTool.waitForElementToBeClickable(driver, tblTds.get(20), interval);
	        action.click(Common.FindAllElements(GlobalRepo.tblTd,20).get(20))
	        .build().perform();
	}
	...
	public static void waitForElementPresentAndDisplay(By element, int timeOutInSeconds) throws Exception{
		WaitTool.waitForElementPresentAndDisplay(driver,element, timeOutInSeconds);
	}
	public static void waitForPageLoadToComplete(By element, int timeOutInSeconds) throws Exception{
		WaitTool.waitForPageLoadToComplete(driver,timeOutInSeconds);
	}
	...
	//	 To click element using JavaScritp 
	...
	// To click element using pixel points
		 ...
		 public static void SwitchToIframe(By checkElement)
		 {  
		  try {   
		   List<WebElement> iframesList = driver.findElements(By.tagName("iframe"));
		   System.out.println(iframesList);
		   for(int i=0; i<iframesList.size(); i++){
		    driver.switchTo().frame(i);
		    if(driver.findElements(checkElement).size()>0)
		     break;
		    driver.switchTo().defaultContent();
		   }
		  }
		 ...
		 public static void selectFormChk(String formNames[])
		 {
			 driver.findElement(By.xpath("//div[@class='slds-col slds-size--11-of-12 ng-binding'][contains(.,'"+formNames+"')]/preceding-sibling::div[position()=1]//input/following-sibling::span")).click();
		 }
